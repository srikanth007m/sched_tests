...
/* ftrace_so_task_timeslice_histogram - give histogram of the timeslice used */
...
static void sched_switch(void * data,int cpu, double time, long long *number, const char **str)
{
	...
	if ((prev_pid != 0) && (task->time[cpu] != -HUGE_VAL))
	{
...
{
	return "This libraries does statistics about timeslice on each cpu. Failing if one timeslice is > 30ms";
}
